-- Zonex Executor v4.1 (0.03 Fast Stable Edition)
    ...
    task.spawn(function()
        while farming do
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                for _, v in pairs(workspace:GetDescendants()) do
                    if v:IsA("Humanoid") and v.Parent and v.Parent ~= char then
                        ...
                        if hrp and v.Health > 0 then
                            ...
                            -- Filter Dummy health 90, 91, 92
                            local isDummy = string.find(name, "Dummy")
                            local badHealth = (h == 90 or h == 91 or h == 92)
                            
                            if not (isDummy and badHealth) then
                ...
                if targetPos then
                    char.HumanoidRootPart.CFrame = CFrame.new(targetPos + Vector3.new(0, 8, 0))
                end
            end
            task.wait(0.03) -- Updated to requested 0.03
        end
    end)
end)
